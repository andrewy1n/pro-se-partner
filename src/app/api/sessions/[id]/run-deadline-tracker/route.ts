...
import { dispatchDeadlineTracker } from "@/lib/agent-dispatcher";
...
export async function POST(
  ...
  try {
    ...
    const handle = await dispatchDeadlineTracker({
      appSessionId: id,
      caseContext,
    });
    return NextResponse.json(handle);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to start Deadline Tracker";
    logServerError("run_deadline_tracker_failed", err, { appSessionId: id });
