...
import type { CanonicalCaseContext, DispatchWave1Response } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  let body: { sessionId?: unknown; caseContext?: unknown };
  try {
    body = (await request.json()) as { sessionId?: unknown; caseContext?: unknown };
  } catch {
    return NextResponse.json({ error: "Expected JSON body" }, { status: 400 });
  }

  const { sessionId, caseContext } = body;
